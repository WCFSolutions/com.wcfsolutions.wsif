...
<div id="entryContent" class="tabMenu">
	<ul>
		...
		<li{if $activeTabMenuItem == 'entryComments'} class="activeTabMenu"{/if}><a href="index.php?page=EntryComments&amp;entryID={@$entry->entryID}{@SID_ARG_2ND}"><img src="{icon}entryCommentM.png{/icon}" alt="" /> <span>{lang}wsif.entry.menu.link.entryComments{/lang}{if $entry->comments} ({#$entry->comments}){/if}</span></a></li>
